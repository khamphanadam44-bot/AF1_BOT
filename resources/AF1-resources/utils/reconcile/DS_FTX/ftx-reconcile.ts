...
/**
 * ตารางค้นหา Actual Row โดยใช้ Matching Key
 *
 * Key:
 * Matching Key จาก Report
 *
 * Value:
 * Actual Row ทุกแถวที่ใช้ Matching Key เดียวกัน
 */
type ActualRowIndex =
 ...
 * - ไม่ลบช่องว่างภายใน
 */
const normalizeMatchingKeyText = (
  value: unknown,
): string => {
  if (
    value === null ||
...
/**
 * สร้าง Index ของ Actual Row
 *
 * Matching Key ว่าง:
 * - จะไม่ถูกเพิ่มลงใน Index
 *
 * Matching Key ซ้ำ:
 * - เก็บทุกแถวไว้ใน Array เดียวกัน
 * - ใช้ตรวจ Duplicate ภายหลัง
 */
const buildActualRowIndex = (
  ...
  for (
    ...
    /**
     * ถ้าพบ Matching Key เดิม
     * ให้เพิ่มแถวต่อท้ายเพื่อใช้ตรวจ Duplicate
     */
    existingRows.push(
      actualRow,
    );
...
/**
 * ค้นหา Actual Row ทั้งหมดด้วย Matching Key
 *
 * ถ้า Matching Key ว่างหรือค้นหาไม่พบ
 * จะคืน Array ว่าง
 */
const findActualRowsByMatchingKey = (
...
/** สร้างผล FAIL เมื่อ Matching Key ใน Test Data ว่าง */
...
/** สร้างผล PASS เมื่อเข้า Exclusion Rule */
...
/** สร้างผล FAIL เมื่อไม่พบ Matching Key ใน Report */
...
/** สร้างผล FAIL เมื่อ Matching Key ซ้ำใน Report */
...
/**
 * จับคู่ Expected Row กับ Actual Row
 * และสร้างผล Compare ทั้งหมด
 *
 * ลำดับการทำงาน:
 * 1. ตรวจ Matching Key ว่าง
 * 2. ตรวจ Exclusion Rule
 * 3. ค้นหา Matching Key ใน Report
 * 4. ตรวจกรณีไม่พบหรือพบซ้ำ
 * 5. เปรียบเทียบ Core Fields
 */
const compareFtxRows = (
  expectedRows: ExpectedRow[],
  actualRows: ActualRow[],
): CompareResult[] => {
  /**
   * สร้าง Index สำหรับค้นหา Matching Key
   */
  ...
  for (
    ...
    /**
     * ป้องกันกรณีไม่พบข้อมูลที่ตำแหน่งแรก
     */
    if (
      actualRow === undefined
    ) {
 ...
 * เพราะ ftx-result-writer.ts จะเป็นผู้สร้างไฟล์
 *
 * หมายเหตุ
 ...
 * โดยใช้ค่าเริ่มต้นของ ftx-row-builder.ts
 */
